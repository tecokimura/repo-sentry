...
ENTRYPOINT ["deno", "run", "--allow-read=/app,/workspace", "--allow-write=/workspace/reports,/workspace/.repo-sentry", "--allow-env=GITHUB_TOKEN,SLACK_WEBHOOK_URL,OPENAI_API_KEY,DENO_DIR,TRIVY_CACHE_DIR", "--allow-net=api.github.com", "--allow-run=gitleaks,trivy", "/app/src/cli.ts"]
