# GitHub Actions 動作確認手順（アーカイブ）

> **このドキュメントは 2026-07-11 に動作確認が完了しました。**
> 現在の状態・次の優先課題は [docs/phase.md](phase.md) を参照してください。
> 手動実行方法は以下に残します（Secrets 登録 → Actions タブ → Run workflow の流れは同じです）。
